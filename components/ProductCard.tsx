    ...
    <div className="group flex flex-col bg-white border border-slate-100 rounded-[32px] overflow-hidden hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] transition-all duration-700 hover:-translate-y-2">
      <div className="aspect-square p-8 flex items-center justify-center relative bg-slate-50/50 overflow-hidden">
        ...
        <div className="absolute top-4 right-4">
          <span className="text-xs font-medium bg-white/90 px-3 py-1.5 rounded-full shadow-sm text-slate-500 border border-slate-100 backdrop-blur-sm capitalize">
            {product.category}
          </span>
        </div>
      </div>
      ...
      <div className="p-6 flex flex-col space-y-4">
        <h3 className="font-semibold text-sm text-slate-800 leading-snug min-h-[2.5rem]">
          {product.name}
        </h3>

        <div className="flex items-center justify-between">
          <p className="text-black font-bold text-lg">
            <span className="text-xs text-slate-400 mr-1 font-normal">KSh</span>
            {product.price.toLocaleString()}
          </p>
          <button
            onClick={() => onAddToCart(product)}
            className="w-11 h-11 bg-black text-white rounded-2xl flex items-center justify-center transition-all duration-300 hover:bg-primary-gold hover:text-black shadow-lg"
          >
            <Plus size={20} />
          </button>
        </div>
